...
class ReadMCC:
    ...
    def process_data(self):
        ...
        if self.array_profiles:
            if self.file_type == 'mcc':
                detectors = DetectorArray(self.meta_data[0], self.clean, normalise_profile=self.normalise_profile, scale=self.scale,
                                          smoothing_factor=self.smoothing_factor)
            else:
                detectors = DetectorArray(self.meta_data, self.clean, normalise_profile=self.normalise_profile, scale=self.scale,
                                          smoothing_factor=self.smoothing_factor)
            ...
            for i in range(0, len(self.meta_data)):
                dataset = pd.DataFrame({'Position': self.clean[i][:, 0], 'Values': self.clean[i][:, 1]})
                if not self.array_profiles:
                    dataset = self.up_sample(dataset, self.scale)
                self.meta_data[i].append(dataset)

                if self.meta_data[i][0] == 'PDD':
                    if self.energy:  # Added for electrons where all energies are in one file
                        if self.meta_data[i][1]['ENERGY'] == self.energy:
                            _mcc_data[self.meta_data[i][0]] = PDD(self.meta_data[i], self.normalise_pdd, self.ion_to_dose).results
                    else:
                        _mcc_data[self.meta_data[i][0]] = PDD(self.meta_data[i], self.normalise_pdd, self.ion_to_dose).results
                if self.meta_data[i][0] == 'INPLANE_PROFILE':
                    if self.meta_data[i][1]['SCAN_DEPTH'] in self.depths:
                        _mcc_data[self.meta_data[i][0] + '_' + str(round(self.meta_data[i][1]['SCAN_DEPTH']))] = XyProfile(self.meta_data[i], self.normalise_profile).results
                if self.meta_data[i][0] == 'CROSSPLANE_PROFILE':
                    if self.meta_data[i][1]['SCAN_DEPTH'] in self.depths:
                        _mcc_data[self.meta_data[i][0] + '_' + str(round(self.meta_data[i][1]['SCAN_DEPTH']))] = XyProfile(self.meta_data[i], self.normalise_profile).results
    ...
    @staticmethod
    def calc_gamma(reference: dict, evaluation: dict, **kwargs) -> dict:
        """
            Calculate an interpolated "x" value between two points.
        ...
        if reference and evaluation:
            ...
            e = list(_evaluation.keys())
            r = list(_reference.keys())
            matches = [x for x in e if x in r]

            gamma_analysis = {}
            for match in matches:
                key_ref, val_ref = match, _reference[match]
                key_eval, val_eval = match, _evaluation[match]
